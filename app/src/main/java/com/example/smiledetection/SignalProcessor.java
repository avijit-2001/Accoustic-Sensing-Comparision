...
public class SignalProcessor extends AppCompatActivity {
    private  Integer lineChartTime = 0;
    ...
    DataAmplitudePhase FourierTransform(double[] chirp, double[] direct, double[] record)
    {
        lineChartTime++;
...
//        Intermediate
        ...
        return new DataAmplitudePhase(lineChartTime, value.abs(), value.getArgument());
    }
    ...
    int binIndex()
    {
        ...
        for(int j = 0; j<SAMPLE_LENGTH/2; j++)
        {
            ...
            if(maxRange1<(maxPhase - minPhase)){
                maxRange1=(maxPhase - minPhase);
                bin=j;
            }
        }
